...
namespace CurrencyBot.Models
{
    ...
    {
        private static Quotes myQuotes;
        private static DateTime LastRefresh;
        private const int MAXTRY = 3;
        private static int attempt = MAXTRY;
        public static Quotes USDQuotes
        {
            get
            {
                InitAll();
        ...
        public static float GetCurrency(string curr)
        {
            InitAll();
            try
            {
                if (myQuotes == null)
                    return (float)1.0;
                string currencysearch = "USD" + curr;
                var ret = (float)myQuotes.GetType().GetProperty(currencysearch).GetValue(myQuotes, null);
                return ret;
            }
            catch (Exception)
            {
        ...
        private static void RefreshCurrencyList()
        {
            //http://www.apilayer.net/api/live?access_key=YOURKEY
            try
            {
                string url = $"http://www.apilayer.net/api/live?access_key={ConfigurationManager.AppSettings["apilayer_key"]}";
                var strret = ExecuteRequest(url);
                var retobjdeser = Newtonsoft.Json.JsonConvert.DeserializeObject<Exchangerates>(strret);
                if (retobjdeser.quotes != null)
                {
                    myQuotes = retobjdeser.quotes;
